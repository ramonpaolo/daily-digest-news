...
- `LEARNING_TOPICS` contains broad domains. Built-in defaults are mathematics, history of mathematics, computing, Go, data structures and algorithms, PostgreSQL, databases, operating systems, and physics.
- The model returns both `topic` (the configured domain) and `subject` (a concrete lesson subject). Lessons favor history/biography for mathematics and internal mechanisms for computing, databases, Go, and operating systems; administrative, management, career, marketing, product, deployment, and cost topics are disallowed.
...
`llm.LessonRequest` carries the configured topic and slot. `llm.Lesson` contains `title`, `topic`, `subject`, `kind`, `opening`, `content`, conditional `answer`, and 3–5 `takeaways`. The client rejects mismatched topics, generic or administrative subjects, unsupported kinds, incomplete answers, oversized fields, and invalid takeaway counts.

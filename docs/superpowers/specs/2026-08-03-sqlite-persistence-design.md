...
- Only learning data is persisted in this iteration: configured interests, request snapshots, complete generated lessons (including concrete `subject` and configured domain), and delivery status.
...
Each learning run creates a `generating` record, stores the complete lesson as `email_pending` before SMTP, and then marks it `sent` or `failed`. Migration version 2 adds `learning_lessons.subject` and backfills legacy rows from `title`, preserving history. SQLite open/migration errors stop startup to prevent silent ephemeral execution. Runtime learning failures remain independent from the news runner.

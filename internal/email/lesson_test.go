...
func TestRenderLessonBuildsSeparateReadableEmail(t *testing.T) {
	...
	if !strings.Contains(message.Subject, "Lição de aprendizado") || !strings.Contains(message.Subject, "estruturas de dados") {
...
func TestRenderLessonOmitsAnswerForText(t *testing.T) {
	message, err := RenderLesson(llm.Lesson{
		Title: "Memória virtual", Topic: "internals de sistemas operacionais", Kind: "text",

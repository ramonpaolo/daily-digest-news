...
func (c *Client) Key() string { return "hacker_news" }

func (c *Client) Name() string { return "Hacker News" }
...
func (c *Client) TopStories(ctx context.Context, limit int) ([]Story, error) {
	...
	for _, id := range ids {
		...
		stories = append(stories, Story{
			ID:         fmt.Sprintf("%s:%d", c.Key(), item.ID),
			Source:     c.Key(),
			SourceName: c.Name(),
			Title:      item.Title,
			URL:        item.URL,
			...
			Text:       item.Text,
			Score:      intPointer(item.Score),
			Comments:   intPointer(item.Descendants),
		})
	}
	if len(stories) == 0 {

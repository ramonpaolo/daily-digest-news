...
func (r *Runner) run(ctx context.Context, force bool) (runErr error) {
	...
	for _, story := range stories {
		...
		if story.URL != "" {
			...
			r.logf("component=job event=article_extract_start story_id=%s source=%s host=%s", story.ID, story.Source, storyHost(story.URL))
			if extracted, extractErr := retry(r, ctx, "article_extract", func(ctx context.Context) (string, error) {
				...
				r.logf("component=job event=article_extract_success story_id=%s chars=%d duration_ms=%d", story.ID, len([]rune(extracted)), time.Since(extractStarted).Milliseconds())
			} else {
				r.logf("component=job event=article_extract_fallback story_id=%s chars=%d duration_ms=%d error=%q", story.ID, len([]rune(story.Text)), time.Since(extractStarted).Milliseconds(), safeError(extractErr))
			}
		} else {
			r.logf("component=job event=article_extract_skipped story_id=%s reason=no_url chars=%d", story.ID, len([]rune(story.Text)))
		}

...
func TestClientSummarizesStoriesUsingZenifraCompatibleEndpoint(t *testing.T) {
	...
	digest, err := client.Summarize(context.Background(), []Input{{ID: "hacker_news:1", Source: "hacker_news", SourceName: "Hacker News", Title: "Title", URL: "https://example.test", ArticleText: "Article"}})
	if err != nil {
		t.Fatalf("Summarize() error = %v", err)
	}
	if digest.Intro != "Abertura" || len(digest.Items) != 1 || digest.Items[0].StoryID != "hacker_news:1" {
...
func TestClientRepairsResponseMissingStory(t *testing.T) {
	...
	client := NewClient(server.Client(), server.URL, "test-key", "test-model")
	digest, err := client.Summarize(context.Background(), []Input{{ID: "hacker_news:1", Title: "Title"}})
	if err != nil {
		t.Fatalf("Summarize() error = %v, want missing story repaired", err)
	}
	if len(digest.Items) != 1 || digest.Items[0].StoryID != "hacker_news:1" {
...
func TestClientLogsResponseMetadataWhenCompletionHasNoContent(t *testing.T) {
	...
	_, err := client.Summarize(context.Background(), []Input{{ID: "hacker_news:1", Title: "Title"}})
...
func TestDecodeStoryIDAcceptsIntegralRepresentations(t *testing.T) {
	for _, test := range []struct {
		...
		t.Run(test.name, func(t *testing.T) {
			...
			if got != test.want {
				t.Fatalf("decodeStoryID(%s) = %q, want %q", test.raw, got, test.want)
			}
		})
	}
}
...
func TestRepairDigestFillsMissingAndIncompleteItems(t *testing.T) {
	inputs := []Input{{ID: "hacker_news:1", Title: "Primeira notícia"}, {ID: "ieee_spectrum:a", Title: "Segunda notícia"}}
	digest := Digest{Items: []Item{{StoryID: "hacker_news:1"}}}
	...
	if !strings.Contains(digest.Items[0].Summary, "Primeira notícia") || digest.Items[1].StoryID != "ieee_spectrum:a" {
